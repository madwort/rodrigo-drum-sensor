...
print(f"time deltas: {time_deltas_seconds}")
...
def find_quadrant(my_list):
    ...
    starting_positions = [(-0.05, 0.05), (0.05, 0.05), (0.05, -0.05), (-0.05, -0.05)]

    posn = _min_position(my_list)
    return (quadrants[posn], starting_positions[posn])

quadrant, quadrant_starting_point = find_quadrant(time_deltas_seconds)

def calculate_params(td_1, td_2):
...
a, b = calculate_params(time_deltas_seconds[0], time_deltas_seconds[1])
c, d = calculate_params(time_deltas_seconds[1], time_deltas_seconds[2])
e, f = calculate_params(time_deltas_seconds[2], time_deltas_seconds[3])
g, h = calculate_params(time_deltas_seconds[3], time_deltas_seconds[0])
